...
class Sotrudniki_otdela(Sotrudnik):
    def __init__(self,ID_S=None,Fam=None,Stl_otd=None):
        self.ID_S=ID_S
        self.Fam=Fam
        self.Stl_otd=Stl_otd

    def print_s_o(self):
        print("{0}  {1}  {2}".format(self.ID_S,self.Fam,self.Stl_otd))


# ФУНКЦИИ
#__________________________________________________________________________

def print_sotr_na_A(stl_sot):
...
for i in range(10):
    d=[]
    d.append(random.choice(id_o)); o2=random.choice(id_o)
    if d[0]!=o2:
        d.append(o2)
    sot.append(Sotrudniki_otdela(i,(random.choice(fam_s)),d))
...
for x in sot:
    if 0 in x.Stl_otd: otd0.append(x)
    if 1 in x.Stl_otd: otd1.append(x)
    if 2 in x.Stl_otd: otd2.append(x)
